...
import { addJSDocComment, NEVER, TRUE, oapiRef, QUESTION_TOKEN, tsModifiers, tsPropertyIndex } from "../lib/ts.js";
...
export default function transformOperationObject(
  ...
  if (operationObject.security) {
    const securityItem = operationObject.security.find((item) => !!(item as any).bearerAuth);
    if (securityItem) {
      type.push(
        ts.factory.createPropertySignature(
          /* modifiers     */
